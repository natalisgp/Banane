...
public class EliminarBibliotecario extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					EliminarBibliotecario frame = new EliminarBibliotecario();
	...
	public EliminarBibliotecario() {
		...
		label_2.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				EliminarBibliotecario.this.dispose(); //hago "invisible la clase login"
		...
		btnEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(ndni.length()>0){ // y exista dicho dni
					...
					try{
						String[] comprobar = funciones.getBibliotecario(ndni);
						
						if(ndni.equals(comprobar[2])){
							
							//new ().setVisible(true);
							EliminarBibliotecario.this.dispose();
							System.out.println("Voy a eliminar al bibliotecario");
							funciones.removeBibliotecario(ndni);
							
							JOptionPane.showMessageDialog(null, "Se ha eliminado el bibliotecario.");
