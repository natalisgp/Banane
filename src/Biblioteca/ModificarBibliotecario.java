...
public class ModificarBibliotecario extends JFrame {

	private JPanel contentPane;
	private JTextField textField_1;
	static String ndni;
	static String npermisos;
	/**
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModificarBibliotecario frame = new ModificarBibliotecario();
	...
	public ModificarBibliotecario() {
		...
		label_2.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				ModificarBibliotecario.this.dispose(); //hago "invisible la clase login"
		...
		btnEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(ndni.length()>0){ // y exista dicho dni
					...
					try{
						String[] usuario = funciones.getBibliotecario(ndni);
						
						if(ndni.equals(usuario[2])){
							
							new ModificarBibliotecario1().setVisible(true);
							ModificarBibliotecario.this.dispose();
							System.out.println("Voy a modificar los datos de un bibliotecario");
